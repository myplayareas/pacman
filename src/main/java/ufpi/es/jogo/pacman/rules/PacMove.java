...
package ufpi.es.jogo.pacman.rules;
...
class PacMove
{
	...
	throws Error
	{
		...
		while (true) 
		{
			...
			{
				if ( cPac.mazeOK(cPac.iX/16 + TablesControl.iXDirection[iHDir],
						cPac.iY/16 + TablesControl.iYDirection[iHDir]) )
					return(iHDir);
			}
			else
				return(iHDir);
	...
	void AddGhostScore()
	{
		...
		for (int i=0; i<4; i++)
		{
			...
			else
			{
				...
				if (iXDis >= 0)
				{
					iDirScore[TablesControl.LEFT] += iXFact;
				}
				else
				{
					iDirScore[TablesControl.RIGHT] += -iXFact;
				}

				if (iYDis >= 0)
				{
					iDirScore[TablesControl.UP] += iYFact;
				}
				else
				{
					iDirScore[TablesControl.DOWN] += -iYFact;
				}
			}
		}
	}
	...
	void AddDotScore()
	{
		...
		if (iX > 0)
		{
			iDirScore[TablesControl.RIGHT] += iFact;
		}
		else if (iX < 0)
		{
			iDirScore[TablesControl.LEFT] += iFact;
		}

		if (iY > 0)
		{
			iDirScore[TablesControl.DOWN] += iFact;
		}
		else if (iY<0)
		{
			iDirScore[TablesControl.UP] += iFact;
		}
	}

...
import ufpi.es.jogo.pacman.rules.Speed;
import ufpi.es.jogo.system.Utility;
...
public class Ghost
{
	...
	public Ghost(Window a, Graphics g, Maze m, Color color)
	{
		applet=a;
		graphics=g;
		maze=m;
		...
		ImageBuilder.drawGhost(imageGhost, 0, color);

		imageBlind=applet.createImage(18,18);
		ImageBuilder.drawGhost(imageBlind,1, Color.white);

		imageEye=applet.createImage(18,18);
		ImageBuilder.drawGhost(imageEye,2, Color.lightGray);
	}
	...
	public void move(int iPacX, int iPacY, int iPacDir)
	{
		...
		{	
			iX+=2* TablesControl.iXDirection[iDir];
			iY+=2* TablesControl.iYDirection[iDir];
		}
	...
	throws Error
	{
		...
		for (i=0; i<4; i++)
		{
			iM=maze.iMaze[iY/16 + TablesControl.iYDirection[i]]
			              [iX/16 + TablesControl.iXDirection[i]];
			if (iM!=Maze.WALL && i != TablesControl.iBack[iDir] )
			{
				iDirTotal++;
			}
		}
	...
	throws Error
	{
		...
		for (i=0; i<4; i++)
		{
			...
			iM=maze.iMaze[iY/16 + TablesControl.iYDirection[i]]
			              [iX/16+ TablesControl.iXDirection[i]];
			if (iM!=Maze.WALL && i!= TablesControl.iBack[iDir] && iM!= Maze.DOOR )
	...
	public void blind()
	{
		if (iStatus==BLIND || iStatus==OUT)
		{
			...
			if (iX%16!=0 || iY%16!=0)
			{
				iDir= TablesControl.iBack[iDir];
				// a special condition:
				// when ghost is leaving home, it can not go back
				// while becoming blind
				int iM;
				iM=maze.iMaze[iY/16+ TablesControl.iYDirection[iDir]]
				              [iX/16+ TablesControl.iXDirection[iDir]];
				if (iM == Maze.DOOR)
					iDir=TablesControl.iBack[iDir];
			}
		}
	...
	throws Error
	{
		...
		{
			iDirCount[i]=0;
			iM=maze.iMaze[iY/16+ TablesControl.iYDirection[i]][iX/16+ TablesControl.iXDirection[i]];
			if (iM != Maze.WALL && i != TablesControl.iBack[iDir] && iM != Maze.DOOR)
				// door is not accessible for OUT
			{
				iDirCount[i]++;

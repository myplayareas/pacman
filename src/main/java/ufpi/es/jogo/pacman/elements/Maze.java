...
public class Maze
{
	...
	public static final int iHeight=HEIGHT*16;
	public static final int iWidth=WIDTH*16;
	...
	// initialize the maze
	public Maze(Window a, Graphics g)
	{
		// setup associations
		applet=a;
		graphics=g;
	...
	public void start()
	{
		...
		for (i=0; i<HEIGHT; i++)
			for (j=0; j<WIDTH; j++)
			{
				switch (TablesControl.MazeDefine[i].charAt(j))
				{
				case ' ':
					k=BLANK;
					break;
	...
	void DrawWall(Graphics g)
	{
		...
		for (i=0; i<HEIGHT; i++)
		{
			for (j=0; j<WIDTH; j++)
			{
				for (iDir=TablesControl.RIGHT; iDir<=TablesControl.DOWN; iDir++)
				{
					if (iMaze[i][j]==DOOR)
					{
					...
					switch (iDir)
					{
					case TablesControl.UP:
						if (i==0)	break;
						if (iMaze[i-1][j]==WALL)
							break;
						DrawBoundary(g, j, i-1, TablesControl.DOWN);
						break;
					case TablesControl.RIGHT:
						if (j==WIDTH-1)	break;
						if (iMaze[i][j+1]==WALL)
							break;
						DrawBoundary(g, j+1,i, TablesControl.LEFT);
						break;
					case TablesControl.DOWN:
						if (i==HEIGHT-1)	break;
						if (iMaze[i+1][j]==WALL)
							break;
						DrawBoundary(g, j,i+1, TablesControl.UP);
						break;
					case TablesControl.LEFT:
						if (j==0)	break;
						if (iMaze[i][j-1]==WALL)
							break;
						DrawBoundary(g, j-1,i, TablesControl.RIGHT);
						break;
					default:	
					}
				}
